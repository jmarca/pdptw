...
// coordinates of the nodes.
  auto Travel = [](const Coordinates* const coords, RoutingModel::NodeIndex from,
...
// service times.
  auto ServiceTime = [](const std::vector<int64>* const service_times,
...
bool LoadAndSolve(const std::string& pdp_file) {
  ...
  // setup callback to Travel function, defined above, to get OD travel time
  routing.SetArcCostEvaluatorOfAllVehicles(
      NewPermanentCallback(Travel, const_cast<const Coordinates*>(&coords)));
  // setup callback to Demand function, defined above, to get node's demands
  // not sure why Demand is dereferences with & here
