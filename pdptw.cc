...
bool LoadAndSolve(const std::string& pdp_file) {
  ...
  for (int line_index = 1; line_index < lines.size(); ++line_index) {
    ...
    // This makes it so demand + delivery always sum to
    // zero, so that when the item is dropped off at the destination,
    // the vehicle capacity can increase accordingly.  For examle,
    // pickup 5 at node 1, pickup 10 at node 2, then vehicle is
    // holding 15.  If drop off at customer 2's destination is -10,
    // the the accumulator will decrement by 10 (add -10 to quantity)
  ...
  // pairs with line 631 in eighth.py
  RoutingModel routing(num_nodes, num_vehicles, depot);

  // pairs with line 655 in eighth.py
  ...
  // this loop adds pick up and delivery constraints
  for (RoutingModel::NodeIndex i(0); i < num_nodes; ++i) {
    // each node is a pickup and has a corresponding delivery, OR it
    // is a depot
    const int64 index = routing.NodeToIndex(i);
    if (pickups[i.value()] == 0
        // for pickup orders, the pickup index is zero
        &&
        deliveries[i.value()] != 0 // the index of the delivery is not the depot
        ){
      // the destination of the delivery, as an integer index
      const int64 delivery_index = routing.NodeToIndex(deliveries[i.value()]);

      // equality constraint says the same vehicle must do both the
      // pickup from node i and the corresponding delivery to the
      // deliveries[i.value()] destination
      solver->AddConstraint(solver->MakeEquality(
          routing.VehicleVar(index), routing.VehicleVar(delivery_index)));

      // less or equal constraint says that the pickup must happen
      // before the delivery, because the cumulative time at the point
      // of pickup must be less or equal to the cumulative time at the
      // point of delivery, and the prior constraint makes sure it is
      // the same vehicle as well.
      solver->AddConstraint(
          solver->MakeLessOrEqual(time_dimension.CumulVar(index),
                                  time_dimension.CumulVar(delivery_index)));

      // not sure what this does, and why it can't automatically fold
      // in the above constraints.
      // ah
      // from the c++ source code comments:
      //
      // Notifies that node1 and node2 form a pair of nodes which
      // should belong to the same route. This methods helps the
      // search find better solutions, especially in the local search
      // phase.
